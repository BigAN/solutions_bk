    ...
    with tick_tock("start add feature"):
        for n, af in enumerate(add_feas_files):
            ...
            if len(af) > 3:
                ...
                to_append = pd.read_feather("train_" + af + "_1_179903891_False_False.ftr")
                ...
                to_append = pd.read_feather("test_" + af + "_1_179903891_False_False.ftr")[be_added]
                check_join(test_df, to_append)
                test_df = test_df.join(to_append[be_added])
                gc.collect()
                to_append = pd.read_feather("val_" + af + "_1_179903891_False_False.ftr")[be_added]
...
with tick_tock("Training..."):
    ...
    params = {'num_leaves': 63, 'scale_pos_weight': 200, 'min_sum_hessian_in_leaf': 30.935343666953045,
              ...
              'bagging_fraction': 0.8266534855846595, 'max_depth': 7, 'feature_fraction': 0.5376703044209775}
    # params = {'num_leaves': 89.0, 'min_sum_hessian_in_leaf': 17.67648456042799, 'learning_rate': 0.11778764435599508,
    #           'seed': 71187, 'min_data_in_leaf': 128.0, 'bagging_fraction': 0.8572042277032333, 'max_depth': 6.0,
    #           'feature_fraction': 0.5067633043826569}
    ...
    with tick_tock("predicting..."):
        print("Predicting...")
        sub['is_attributed'] = bst.predict(test_df[predictors].values.astype('float32'), num_iteration=best_iteration)
