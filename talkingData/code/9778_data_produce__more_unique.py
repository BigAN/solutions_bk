...
def DO(frm, to, fileno):
    ...
    with tick_tock("next features"):
        ...
        train_df = do_countuniq(train_df, ['ip', 'day'], 'period');gc.collect();
        train_df = do_countuniq(train_df, ['ip', 'device'], 'channel');gc.collect();

        train_df = do_countuniq(train_df, ['ip', 'app'], 'device');gc.collect();
        train_df = do_countuniq(train_df, ['ip', 'os'], 'device');gc.collect();
        train_df = do_countuniq(train_df, ['ip', 'device', 'os','app'], 'period');gc.collect();
