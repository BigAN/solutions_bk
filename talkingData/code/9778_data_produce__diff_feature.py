...
def do_prev_Click(df, agg_suffix='prevClick', agg_type='float32'):
    ...
    for spec in GROUP_BY_NEXT_CLICKS:
        ...
        df['next_prev_' + new_feature] = (df['next'] - \
                                          df[all_features].groupby(spec['groupby'])['next'].shift(+1)).astype(agg_type)
        df['next_next_' + new_feature] = (df[all_features].groupby(spec['groupby'])['next'].shift(-1) - \
                                          df['next']).astype(agg_type)
        df['next_next_' + new_feature + "_mean"] = (df[['next_next_' + new_feature] + all_features].groupby(spec['groupby'])[
