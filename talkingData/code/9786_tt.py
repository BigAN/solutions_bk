...
def do_next_Click( df,agg_suffix='nextClick', agg_type='float32'):
    ...
    for spec in GROUP_BY_NEXT_CLICKS:
       ...
       # Name of new feature
        ...
        df[new_feature + "_o"] = (df[all_features].groupby(spec[
            'groupby']).click_time.shift(-1) - df.click_time)
        df[new_feature + "_r"] = (df[all_features].groupby(spec[
...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv("/home/dongjian/data/talkingdata/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv("/home/dongjian/data/talkingdata/test.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    else:
        test_df = pd.read_csv("/home/dongjian/data/talkingdata/test.csv", parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    ...
    # train_df.reset_index().to_feather('train_df_9786.ftr')
    # val_df.reset_index().to_feather('val_df_9786.ftr')
    # test_df.reset_index().to_feather('test_df_9786.ftr')
