...
def DO(frm, to, fileno):
    ...
    with tick_tock("next features"):
        ...
        # train_df = do_countuniq(train_df, ['ip'], 'os');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip', 'day'], 'period');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip', 'device'], 'channel');
        # gc.collect();
        #
        # train_df = do_countuniq(train_df, ['ip', 'app'], 'device');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip', 'os'], 'device');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip', 'device', 'os', 'app'], 'period');
        # gc.collect();
        #
        # train_df = do_countuniq(train_df, ['ip', 'channel'], 'app');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip', 'app', 'channel'], 'device');
        # gc.collect();
        #
        # train_df = do_count(train_df, ['ip', 'device', 'os', 'app']);
        # gc.collect()
        # train_df = do_count(train_df, ['ip', 'channel']);
        # gc.collect()
        # train_df = do_count(train_df, ['app', 'channel']);  # +0.0004
        # gc.collect()
        # train_df = do_count(train_df, ['ip', 'os']);
        # gc.collect()
        # train_df = do_count(train_df, ['ip']);  # +0.0004
        # gc.collect()
        #
        # # train_df = do_countuniq(train_df, ['ip'], 'app');
        # # gc.collect();
        ...
        # # gc.collect();
        # #####################################################################
        # train_df = do_cumcount(train_df, ['ip', 'app'], 'channel');
        # gc.collect();
        # train_df = do_cumcount(train_df, ['ip', 'app'], 'device');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['app', 'channel'], 'ip');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['app', 'channel'], 'os');
        # gc.collect();
        # train_df = do_cumcount(train_df, ['app', 'channel'], 'ip');
        # gc.collect();
        # train_df = do_cumcount(train_df, ['app', 'channel'], 'os');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['app', 'channel'], 'period');
        # gc.collect();
        # train_df = do_countuniq(train_df, ['ip'], 'period');
        # gc.collect();

        ##################
        train_df = do_countuniq(train_df, ['channel'], 'app');
        gc.collect()

        train_df = do_count(train_df, ['app', 'os']);
        gc.collect()
        train_df = do_countuniq(train_df, ['app', 'os'], 'channel');
        gc.collect();
        train_df = do_countuniq(train_df, ['app', 'os'], 'device');
        gc.collect();
        train_df = do_countuniq(train_df, ['app', 'os'], 'ip');
        gc.collect();
