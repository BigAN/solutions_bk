...
if __name__ == '__main__':
    ...
    tr.loc[:, 'delivery_distance_k'] = tr.loc[lambda x: x.area_busy_coef > 0, "area_busy_coef"].apply(
        lambda x: int(x) / 2)
    ...
    te.loc[:, 'delivery_distance_k'] = te.loc[lambda x: x.area_busy_coef > 0, "area_busy_coef"].apply(
        lambda x: int(x) / 2)
    ...
    def gene_poi_agg_dict(prefix):
        return {
            ...
            "delivery_distance": {prefix + prefix_sep.join(gby_cols) + "_mean_delivery_distance": "median",
                                  prefix + prefix_sep.join(gby_cols) + "_mean_delivery_distance": "std"},
            "food_total_value": {prefix + prefix_sep.join(gby_cols) + "_mean_food_total_value": "median",
                                 prefix + prefix_sep.join(gby_cols) + "_mean_food_total_value": "std"},
    ...
    for area_id in [0, 1, 2, 3, 4, 5,6]:
        tr.loc[tr['delivery_distance_k'] == area_id].to_csv(
            args.input_path + "train_cv_area_{}.csv".format(str(area_id)))
        te.loc[te['delivery_distance_k'] == area_id].to_csv(
            args.input_path + "test_cv_area_{}.csv".format(str(area_id)))
