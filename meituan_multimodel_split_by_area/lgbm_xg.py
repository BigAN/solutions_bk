...
if __name__ == '__main__':
    ...
    # features = list(set(basic_features) - set(to_drop))
    features = filter(lambda x: "coor_count" not in x,
                      list(set(data.columns.tolist()[1:]) - set(to_drop) - set(fea_drop)))
    print "f ", features, "  e"
    ...
    dtrain = xgb.DMatrix(train_data[features].values, labels)
    dtest = xgb.DMatrix(test_data[features].values,labels_val)
