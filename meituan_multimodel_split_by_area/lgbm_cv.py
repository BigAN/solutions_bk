...
class area_model(object):
    ...
    def train(self):
        ...
        labels_val = self.test_data[[args.label]].values.astype(np.float32).flatten()

        lgb_train = lgb.Dataset(self.train_data[self.features], labels)
        lgb_eval = lgb.Dataset(self.test_data[self.features], labels_val, reference=lgb_train)
    ...
    def predict(self):
        ...
        print "!!!!!!!!!!!!!! rs is {} !!!!!!!!!!!!!!!!!".format(
                mean_absolute_error(self.test_data[args.label], self.test_data["delivery_duration_prd"]))
        print self.test_data["delivery_duration_prd"].describe()
        print self.test_data[args.label].describe()
        self.test_data['mae'] = self.test_data[args.label] - self.test_data["delivery_duration_prd"]
...
if __name__ == '__main__':
    ...
    out.to_csv("/Users/dongjian/data/meituanKaggleData/test_out.csv")
