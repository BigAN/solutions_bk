...
if __name__ == '__main__':
    ...
    for f in [None] + features[:-1]:
        ...
        if f:
            # print "wit drop f {}".format(f)
            best_drop.append(f)

        if best_score < cur_s:
            print "update best score from {} to {}".format(best_score,cur_s)
            best_score = cur_s
            mem_drop = copy.copy(best_drop)

        print "best score is {}, cur_s is {}, ".format(best_score, cur_s)
        log.append([f,cur_s,best_score])

    print "best score is {},mem drop is {}".format(best_score,mem_drop)
    print log

    # print "final score is  {},best drop is {}".format(best_score, best_drop)
