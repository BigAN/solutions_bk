...
print(check_output(["ls", "/Users/dongjian/data/sales_forcasting"]).decode("utf8"))
    ...
    def read_train_test_low_memory(self, train_flag=True):
        ...
        if train_flag:
            data = pd.read_csv(self.DATA_FOLDER + filename + '.csv', parse_dates=['date'], dtype=types,
                               nrows=self.nrows, infer_datetime_format=True, skiprows=range(1, 66458909))
        else:
            data = pd.read_csv(self.DATA_FOLDER + filename + '.csv', parse_dates=['date'], dtype=types,
...
if __name__ == '__main__':
    data = Data(data_folder='/Users/dongjian/data/sales_forcasting/', test_the_script=True)
