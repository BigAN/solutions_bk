...
if __name__ == '__main__':
    ...
    print "origin features, length {},{}".format(len(train_data.columns.tolist()[1:]), train_data.columns.tolist()[1:])
    ...
    test_pool = Pool(test_data[features], label=labels_val)
    ...
    cat = cb.CatBoostRegressor(iterations=args.round,
                               ...
                               verbose=True,
                               eval_metric="MAE",
                               loss_function='MAE').fit(train_pool, eval_set=test_pool,
                                                        cat_features=["poi_id", "area_id", "10min", "15min", "weekday"])

    print cat.get_feature_importance(train_pool)
    test_data.loc[:, "delivery_duration_prd"] = cat.predict(test_pool)
