...
def logloss(ys, ps, neg_ratio=1.):
    ...
    for i in range(len(ys)) :
        ...
        try:
            ...
            print >> sys.stderr, "log error:"+str(y1)+" "+str(y0)+" "+str(y)
...
def preprocess(np_data):
    ...
    #归一化
    trans_dense_fea = 1 / (1 + np.exp(-trans_dense_fea))#sigmoid
...
def get_net(num_vocab, num_embed, dropout=0.0):
    ...
    print >> sys.stderr, net.infer_shape(cx=(1000,CATE_FEA_LEN), dx=(1000,DENSE_FEA_LEN))
    return net
...
#训练data_dir文件夹下的所有文件（test_file）除外
#prefix表示基模型的路径和名称,非增量训练时表示输出模型名前缀
#start表示基模型的checkpoint，非增量训练时表示输出模型checkpoint
#file_num_limit：最多训练的问题件数，默认为0
def train_dir(data_dir, prefix, start, test_file, file_num_limit=0):
    ...
    data_list.sort()
    #加载测试数据
    print >> sys.stderr, "loading test data:" + test_file
    ...
    print >> sys.stderr, "training files:" + ",".join(data_list)
    for i in range(out_loop_num):
        for i,file_name in enumerate(data_list):
            print >> sys.stderr, time.ctime()," training file: ",file_name
            np_data = load_np_data_bin(data_dir + "/" + file_name, False)
            mod = train(net , np_data, prefix, start + i, epoch)
            print >> sys.stderr, "eval: ",evaluate(mod, data_eval)
            del np_data
            gc.collect()
            print >> sys.stderr, "finish %d:%s"%(start + i, file_name)
        start += len(data_list)
    return start
...
if __name__ == '__main__':
    ...
    #训练单个文件
    net = get_net(num_vocab, num_embed)
    np_data = load_np_data_bin('./train_data', False)
    prefix = 'checkpoint'
    mod = train(net, np_data, prefix, 0, 3)
    data_eval = load_np_data_bin("./test_data", True)
