...
def gene_mask(df, h, high=False, do_base=True):
    if do_base:
        ...
        # base = (df.delivery_duration < 5000.0) & (df.delivery_duration > 500.0) & \
...
if __name__ == '__main__':
    ...
    # ori.loc[:, 'delivery_duration_bin'] = ori[ori.delivery_duration>0]["delivery_duration"].apply(lambda x: int(x / 1000) if x != np.nan else 0)
    ...
    kmeans = MiniBatchKMeans(n_clusters=10000, batch_size=10000).fit(coords[sample_ind])
    ...
    # te_cluster_agg = nm.ka_add_groupby_features_1_vs_n(tr, gby_cols, cluster_dict)
    ...
    cur_area_dict = {
        ...
        "_10min_not_fetched_order_num": {"cur_block_10min_not_fetched_order_num": "sum"
                                         },
        "food_total_value": {
            # prefix + prefix_sep.join(gby_cols) + "_food_total_value_mean": "median",
            # "future_food_total_value_std": "std",
            "cur_block_food_total_value_sum": "sum"
        },
        "delivery_distance": {
            # prefix + prefix_sep.join(gby_cols) + "_food_total_value_mean": "median",
            # prefix + prefix_sep.join(gby_cols) + "_food_total_value_std": "std",
            "cur_block_delivery_distance": "sum"
        }
    ...
    cur_poi_area_dict = {
        "area_busy_coef": {
            "cur_block_area_busy_coef": "median"
        },
    ...
    cur_poi_dict = {
        ...
        "waiting_order_num": {"cur_block_waiting_order_num": "sum"},
        "order_id": {"cur_block_order_id_count": "count",
                     }
    }
    ...
    # tr['cur_block_area_busy_coef_change'] = tr['cur_area_busy_coef_y'] - tr['area_busy_coef']
    # te['cur_block_area_busy_coef_change'] = te['cur_area_busy_coef_y'] - te['area_busy_coef']
