...
if __name__ == '__main__':
    ...
    # 高峰期的数据分为 last_week,参与评测 . 参与评测又分为tr ,te
    for_te = ori.loc[gene_mask(ori, 11) & ((ori.day <= 21) | (ori.day == 30) | (ori.day == 22) | (ori.day == 23))]
    tr, te = train_test_split(for_te, test_size=0.5, random_state=2017)

    tr = tr.loc[(ori.day != 30) & (ori.day != 22) & (ori.day != 23)]
    last_week = ori.loc[gene_mask(ori, 11) & ((ori.day >= 24) & (ori.day <= 29))]
    other_high = ori.loc[
        (gene_mask(ori, 12) | gene_mask(ori, 13)) & (ori.day != 30) & (ori.day != 22) & (ori.day != 23)]

    lasthour_for_te = ori.loc[
        gene_mask(ori, 10) & ((ori.day <= 21) | (ori.day == 30) | (ori.day == 22) | (ori.day == 23))]
    pre, ta = train_test_split(lasthour_for_te, test_size=0.5, random_state=2017)
    pre = pre.loc[(ori.day != 30) & (ori.day != 22) & (ori.day != 23)]
    lasthour_last_week = ori.loc[gene_mask(ori, 10) & ((ori.day >= 24) & (ori.day <= 29))]

    tr = pd.concat([tr, other_high, last_week], ignore_index=True)
    pre = pd.concat([pre, lasthour_last_week], ignore_index=True)
    ...
    for gby_cols in [
        # ['10min', 'hour', 'weekday', 'poi_id'],
        ['poi_id', 'hour', 'weekday'],
        ['pickup_cluster', '10min'],
    ...
    for gby_cols in [
        ...
        area_info_dict = {'avg_speed': {prefix_sep.join(gby_cols) + '_mean_dd': 'median'},
                          ...
                          "arrive_guest_cost": {
                              prefix_sep.join(gby_cols) + "_lasthour" + "_mean_arrive_guest_cost": "median"},
                          "order_id": {prefix_sep.join(gby_cols) + "_coor_count": "count",
                                       prefix_sep.join(gby_cols) + "_order_count": "count"},
                          "food_total_value": {prefix_sep.join(gby_cols) + "_mean_food_total_value": "median"},
                          "box_total_value": {prefix_sep.join(gby_cols) + "_mean_box_total_value": "median"},
                          "food_num": {prefix_sep.join(gby_cols) + "_mean_food_num": "median",
                                       prefix_sep.join(gby_cols) + "_total_food_num": "sum"},
                          "waiting_order_num": {prefix_sep.join(gby_cols) + "_waiting_order_num": "median",
                                                prefix_sep.join(gby_cols) + "_waiting_order_num_sum": "sum",
                                                },
                          }
